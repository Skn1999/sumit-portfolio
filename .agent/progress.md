...
| 2026-07-30 | `HeroParticleCanvas 3D "me" Text Node & Silver Metal Solidification` | Replaced legacy 2D image plane gold coin shader with extruded 3D "me" text geometry created via `TextGeometry` & `FontLoader`. Unified 3D surface point sampling (`MeshSurfaceSampler`) and silver metal solidification across all categories. Build passes. | Completed |
